...
function bundleConfig(minify) {
  return {
    ...
    moduleName: 'SwipeToRevealOptions',
    dest: 'dist/swipeToRevealOptions' + (minify ? '.min' : '') + '.js',
    sourceMap: !minify
  }
}
...
rollup.rollup(rollupConfig(process.env.NODE_ENV === 'production')).then(bundle => {
  ...
  console.log('Successfully bundled SwipeToRevealOptions' + (process.env.NODE_ENV === 'production' ? ' (minified).' : '.'))
}).catch(errorOnFail)
